...
public class CBTabBarButton: UIControl {
    ...
    private func configureSubviews() {
        tabImage.contentMode = .left
        tabImage.translatesAutoresizingMaskIntoConstraints = false
        tabLabel.translatesAutoresizingMaskIntoConstraints = false
        tabLabel.font = UIFont(name: "Nunito-Bold", size: 14.0)//UIFont.systemFont(ofSize: 14)
        ...
        if rightToLeft {
            ...
            tabImage.leadingAnchor.constraint(equalTo: tabBg.leadingAnchor, constant: bgHeight/3).isActive = true
            tabImage.centerYAnchor.constraint(equalTo: tabBg.centerYAnchor).isActive = true
            ...
            csUnfoldedLblLeading = tabLabel.leadingAnchor.constraint(equalTo: tabImage.trailingAnchor, constant: bgHeight/10.0)
    ...
    private func fold(animationDuration duration: Double = 0.0) {
        ...
        UIView.transition(with: tabImage, duration: duration, options: [.transitionCrossDissolve], animations: {
            self.tabImage.tintColor =  UIColor(red: 255.0/255, green: 255.0/255, blue: 255.0/255, alpha: 0.5) //UIColor.hexColor(rgbValue: 0x0E5DA3)
        }, completion: nil)
    ...
    private func unfold(animationDuration duration: Double = 0.0) {
        ...
        UIView.animate(withDuration: duration * 0.5, delay: duration * 0.5, options: [], animations: {
            ...
            self.tabLabel.textColor = UIColor.white//hexColor(rgbValue: 0x0E5DA3)
           // self.tabLabel.font = UIFont(name: "Poppins-Medium", size: 14.0)
        }, completion: nil)
        UIView.transition(with: tabImage, duration: duration, options: [.transitionCrossDissolve], animations: {
            self.tabImage.tintColor = UIColor.white//hexColor(rgbValue: 0x0E5DA3)//self.tintColor
        }, completion: nil)
    }
